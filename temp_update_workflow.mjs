import { execSync } from "node:child_process";

const REPO = "alishers-company/freelance-web-site";

// Get current file SHA
const sha = execSync(
  `gh api repos/${REPO}/contents/.github/workflows/agent.yml --jq .sha`,
  { encoding: "utf8" }
).trim();
console.log("Current SHA:", sha);

const newContent = `name: Implementation Agent

on:
  issue_comment:
    types: [created]
  workflow_dispatch:
    inputs:
      issue_number:
        description: Issue number to process
        required: true
        type: string

permissions:
  contents: write
  issues: write

jobs:
  agent:
    if: >-
      github.event_name == 'workflow_dispatch' ||
      contains(github.event.comment.body, '@glm')
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
        with:
          fetch-depth: 0
      - uses: actions/setup-node@v4
        with:
          node-version: 20
      - name: Run GLM-4 implementation
        id: implement
        continue-on-error: true
        env:
          BIGMODEL_API_KEY: \${{ secrets.BIGMODEL_API_KEY }}
          BIGMODEL_BASE_URL: https://open.bigmodel.cn/api/paas/v4/chat/completions
          BIGMODEL_MODEL: glm-4-plus
          GITHUB_TOKEN: \${{ secrets.GITHUB_TOKEN }}
        run: node .github/scripts/implement-with-glm.mjs
      - name: Commit implementation
        if: steps.implement.outcome == 'success' && steps.implement.outputs.changed == 'true'
        run: |
          git config user.name "github-actions[bot]"
          git config user.email "41898282+github-actions[bot]@users.noreply.github.com"
          git add .
          git commit -m "feat: implement issue #\${{ github.event.issue.number || inputs.issue_number }} with GLM-4"
          git push origin HEAD:\${{ github.event.repository.default_branch }}
      - name: Post completion note
        if: steps.implement.outcome == 'success' && steps.implement.outputs.changed == 'true'
        env:
          GH_TOKEN: \${{ github.token }}
          GLM_SUMMARY: \${{ steps.implement.outputs.summary }}
        run: |
          gh issue comment "\${{ github.event.issue.number || inputs.issue_number }}" \\
            --repo "\${{ github.repository }}" \\
            --body "GLM-4 committed the requested implementation directly to the default branch.

          Summary: \$GLM_SUMMARY"
      - name: Post no-change note
        if: steps.implement.outcome == 'success' && steps.implement.outputs.changed != 'true'
        env:
          GH_TOKEN: \${{ github.token }}
          GLM_SUMMARY: \${{ steps.implement.outputs.summary }}
        run: |
          gh issue comment "\${{ github.event.issue.number || inputs.issue_number }}" \\
            --repo "\${{ github.repository }}" \\
            --body "GLM-4 reviewed the issue but did not produce file changes.

          Summary: \$GLM_SUMMARY"
      - name: Post failure note
        if: steps.implement.outcome != 'success'
        env:
          GH_TOKEN: \${{ github.token }}
          GLM_ERROR: \${{ steps.implement.outputs.error }}
        run: |
          gh issue comment "\${{ github.event.issue.number || inputs.issue_number }}" \\
            --repo "\${{ github.repository }}" \\
            --body "GLM-4 could not complete the implementation run.

          Error: \$GLM_ERROR"
`;

const b64 = Buffer.from(newContent, "utf8").toString("base64");
const body = JSON.stringify({
  message: "fix: add workflow_dispatch trigger",
  content: b64,
  sha: sha,
  branch: "main",
});

const result = execSync(
  `gh api repos/${REPO}/contents/.github/workflows/agent.yml -X PUT --input -`,
  { input: body, encoding: "utf8" }
);
const parsed = JSON.parse(result);
console.log("Updated! Commit:", parsed.commit?.sha);
